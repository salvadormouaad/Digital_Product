...
import bg from '../../assets/bi-202120-applemusic-en.jpg'
  ...
  return (
    <div className={`flex justify-center items-center md:w-50 p-0 w-full ${className || ''}`}>
      <div className="group w-full max-w-xs md:max-w-[200px] h-48 md:h-40 overflow-hidden relative 
                     ...
                     hover:duration-500 
                     after:absolute after:w-12 after:h-12 after:bg-[#495BA0] after:rounded-full 
                     after:blur-lg after:bottom-12 after:right-8 
                     before:absolute before:w-10 before:h-10 before:bg-rose-400 before:rounded-full 
        ...
        <div className="z-10 flex flex-col items-center gap-1 p-2 text-center">
          ...
          <span className="text-gray-500 text-base md:text-sm">{Quality}</span>
          <span className="text-white text-xl">{Duration}</span>
          <span className="text-white text-xl md:text-lg font-bold mt-1">{Price}</span>
          <a href='https://wa.me/212620394795?text=Salam%20svp%20le%20prix%20dial%20Apple%20Music%20w%20les%20offres
' target='blank' className="mt-0 px-4 py-1 md:px-3 md:py-1 bg-stone-600 hover:bg-stone-800 
                            text-stone-300 font-semibold rounded-md transition-colors duration-200 
                            text-sm md:text-xs">
            Order Now
          </a>
        </div>
